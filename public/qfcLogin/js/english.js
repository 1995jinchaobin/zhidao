...
function langChange(index){
    if(index==0){
        window.location.href = 'https://www.zhidaoai.com/qfcLogin/index.html?type=1';
    }
}
// 获取数据
...
register.onclick = function(event){
    ...
    localStorage.setItem('English',true);
    localStorage.setItem('path','/Enregister');
    window.location.href = 'http://www.zhidaoai.com';
}
// 忘记密码点击
...
forget.onclick = function(event){
    ...
    localStorage.setItem('English',true);
    localStorage.setItem('path','/Enforget');
    window.location.href = 'http://www.zhidaoai.com';
}
...
btn.onclick = function(event){
    ...
    if(phone.value == '' || pass.value == ''){
        tig.innerHTML = "Necessary cann't be empty";
        model.style.display = 'block';
        setTimeout(function(){
            tig.innerHTML = '';
            model.style.display = 'none';
