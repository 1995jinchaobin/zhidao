      ...
      <div>
        <ul class="productBox1">
          <div class="hbox">
            <div class="hbox2">
              <div style="width: 60px;position: relative;">
                <!--<input class="checkBox" type="checkbox" @click="quanxuan0shuzu()" />-->
                <div :class="{checkBox:true,checkedBox:checkedAllFlag==true}" @click="checkAll()"></div>
              </div>
            </div>
            <div class="hbox1">素材图</div>
            <div class="hbox1 sourceName">素材名称</div>
          ...
          <div v-if="message==''" class="noData">
            <div
              style="margin-top:104px; font-size:16px;text-aligh:center;font-size:16px;color:#D4CFCF"
            >您还未上传作品哦！快上传作品拿第一桶金</div>
            <button
              @click="tianjiazuopin"
              style="margin-top:30px;width:94px;background-color:#FFE135;color:#666666"
            >+ 添加作品</button>
          </div>
          <li
          ...
          >
            <div style="width: 60px;position: relative;top: 59.5px;">
              <!--<input-->
                <!--type="checkbox"-->
                <!--:checked="shuzu.indexOf(item.id)>-1"-->
                <!--@click="shuzuf(item.id)"-->
                <!--:value="item.id"-->
              <!--/>-->
            ...
            <div class="productli1">
              ...
              <img style="border-radius:4px" :src="item.picture" alt />
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    tablesucai() {
      ...
      let obj = {
        url:
          self.all.baseUrl + "/new/materialLibrary/getMaterialLibrarysByUserId",
        formdata: formData
      };
      self.getData(obj).then(res => {
        setTimeout(function() {
