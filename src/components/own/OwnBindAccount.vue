<template>
    <div class="xiugai">
        <div class="header">
            ...
            <div class="tips">(  一个账号只可绑定两个子账号，您还可绑定账号为：{{2-parseInt(memberNum)}}  )</div>
        </div>
        <div class="secondBind" v-if="showBindBox">
            <div class="title">添加子账号</div>
            <div class="inputArea">
                <span class="leftTip">子账号</span>
                <input v-model="number" placeholder="请输入要绑定的账号" type="text" />
            </div>
            <div class="btnBox">
        ...
        <table class="table">
            ...
            <tr v-for="(item, index) in memberList" :key="index">
                <td>{{item.bind_user_phone}}</td>
                <td>{{item.create_time}}</td>
                <td>
                    <button v-if="item.status==0" class="cancelRalate" @click="cancelRalate(index)" ><div class="point"> </div><div class="cancelBtn">取消绑定</div></button>
                    <button v-if="item.status==-10" class="ralate" @click="ralate(index)" ><div class="point"> </div><div class="relateBtn">绑定</div></button>
                </td>
            </tr>
            </tbody>
        </table>
        <button class="upload" @click="submit()" v-if="memberNum<2">+ 添加子账号</button>
        <div class="bottomTips" v-if="memberNum==0">(您还没添加子账号请添加)</div>
    ...
    export default {
        ...
        methods: {
            ...
            quxiao(index){
                ...
                let obj = {
                    url: this.all.baseUrl + "/own/userPicSearch/userPicUnBind",
                    formdata: formData
                };
            ...
            bind(){
                ...
                let obj = {};
                obj.url = this.all.baseUrl + '/own/userPicSearch/userPicBind';
            ...
            getList() {
                ...
                let self = this;
                let obj = {
                    url: this.all.baseUrl + "/own/userPicSearch/getUserPicBinds",
                    formdata: formData
                };
                self.getData(obj).then(res => {
                    if (res.data.status == 0) {
            ...
            sureCancelRalate(){
                ...
                if(self.memberList[self.userIndex].status==-10){
                    obj.url = this.all.baseUrl + "/own/users/vipBindUser"
                    formData.append('bindUserId',self.memberList[self.userIndex].id);
                }else{
                    formData.append('id',self.memberList[self.userIndex].id);
                }
                self.getData(obj).then(res => {
                    if (res.data.status == 0) {
                        self.showLoading=false;
            ...
            sureRalate(){
                ...
                let obj = {
                    url: this.all.baseUrl + "/own/users/vipBindUser",
                    formdata: formData
                };
                self.getData(obj).then(res => {
                    if (res.data.status == 0) {
