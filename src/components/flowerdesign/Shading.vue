        ...
        <div class="content">
            ...
            <div class="backBox">
                ...
                <div v-if="showStyle" class="styleBox">
                    <h4>风格：</h4>
                    <ul class="styleList">
                        <li @click="styleClick(index)" :class="item.show?'special':''" v-for="(item,index) in styleList" :key="index">{{item.name}}</li>
                    </ul>
                </div>
                <ul class="proBox">
...
export default {
    ...
    data(){
        return {
            ...
            height: '',
            showLoading: false,
            leftNav: [{name: '添加素材',show: false},{name: '花型构图',show: false},{name: '添加底纹',show: true},{name: 'AI配色',show: false}],
            navIndex: 0,
            backList: [{name: '共享底纹',show: true},{name: '我的底纹',show: false}],
            type: 0,
            list: [],
            // 页数
            cnt: 1,
                ...
                if(res.data.status==0){
                    if(res.data.result.res){
                        ...
                        this.getData(obj).then(res=>{
                            ...
                            if(res.data.status==0){
                                self.newSave(res.data.result.id);
                            }else if(res.data.status==-95){
                                this.showJump = true;
                                this.err = res.data.msg;
                                Scroll.stop();
