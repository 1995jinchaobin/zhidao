        ...
        <div v-if="land == 1" class="rightUser">
          ...
          <div class="avator" @mouseenter="downChange" @mouseleave="topChange">
            <div class="userImg" @click="linkTo('Flower','')">
                <img class="avatorImg" :src="userHeadPhoto">
            </div>
  ...
  data() {
    return {
      ...
      arr: [
        ...
        { name: "以图识图", show: false, link: "/picture" },
    ...
    auto() {
      ...
      if(this.user){
        if(localStorage.getItem("nickname")){
          ...
          this.userName = '用户' + localStorage.getItem("phone").substr(7,4);
        }
        if(localStorage.getItem("headPhoto")){
          let imgUrl = localStorage.getItem("headPhoto");
          if(imgUrl&&imgUrl != 'null'){
            this.userHeadPhoto = imgUrl;
          }else{
      ...
      if(localStorage.getItem('qfc')){
        ...
        this.getData(obj).then(res=>{
            ...
            if(res.data.status==0){
                ...
                self.userName = localStorage.getItem("nickname");
                if(localStorage.getItem("headPhoto")){
                  let imgUrl = localStorage.getItem("headPhoto");
                  if(imgUrl&&imgUrl != 'null'){
                    self.userHeadPhoto = imgUrl;
                  }else{
                    self.userHeadPhoto = require('../assets/image/user/avator.png');
                  }
                }else{
                  self.userHeadPhoto = require('../assets/image/user/avator.png');
                }
                ...
                let time2 = Scroll.changeDateWay(Number(localStorage.getItem("expirationTimePlus")));
                if(localStorage.getItem("expirationTime") < localStorage.getItem("expirationTimePlus")){
                  self.expirationTime = time2;
                }else{
                  self.expirationTime = time1;
                }
                self.$forceUpdate();
                localStorage.removeItem('qfc');
            }else if(res.data.status==-95){
                self.showJump = true;
      ...
      this.getData(obj).then(res=>{
          ...
          if(res.data.status==0){
              ...
              if(localStorage.getItem('qfc')){
                ...
                localStorage.setItem('headPhoto',result.userInfo.head_photo);
                localStorage.removeItem('qfc');
              }
              localStorage.removeItem('expirationTime');
              localStorage.removeItem('expirationTimePlus');
