<template>
  <div class="window">
    ...
    <div class="studio">
      <div class="click">Sold pattern</div>
    </div>
    <div class="shuru">
      <div class="time">Release time</div>
      <input class="time1" language='english' type="date" v-model="message1c.time1" />
...
export default {
        ...
        if (res.data.status == 0) {
          ...
          for (let i = 0; i < this.message.list.length; i++) {
            ...
            self.message.list[i].genre1 =
              self.select[self.message.list[i].genre1].english;
  ...
  mounted() {
    ...
    this.getselect();
    this.tablesucai();
    localStorage.setItem("path", "/EnTransactionManagement");
