<template>
  <div class="window">
    ...
    <div class="studio">
      <div @click="qiehuan(0)" :class="type==0?'click':''">Upload pattern</div>
      <div @click="qiehuan(1)" :class="type!=0?'click':''">Manage patterns</div>
    </div>
    <div class="shuru" v-if="type==0">
      ...
      <div>
        ...
        <span style="position:relative;left: 500px;">yuan</span>
      </div>
      <div>
        PSD source file
        <input type="file" @change="getFile" accept="image/*" ref="file" id="file" />
        <label class="uploadfile" for="file">Upload file</label>
      </div>
      <div>
        size
        <span style="position:relative;left: 120px;">Long</span>
        ...
        <span style="position:relative;left: 170px;">High</span>
      ...
      <div style="height:150px">
        ...
        <select class="select" v-model="message.lianjie">
          <option>Single picture</option>
          <option>Two-sided continuity</option>
          <option>Quadripartite continuity</option>
        </select>
      </div>
      <div>
        Pattern type
        <select class="select" v-model="message.huatype1">
          <option v-for="item in hualist1" :key="item.id" :value="item.id">{{item.english}}</option>
        </select>
        <select class="select" v-model="message.huatype2" style="top: -40px;
      left: 600px;">
          <option v-for="item in hualist2" :key="item.id" :value="item.id">{{item.english}}</option>
        </select>
      ...
      <div class="flowerimage">
        Pattern picture
        <span style="font-size:12px">You can upload up to 4 slide pictures, each picture is not more than 5MB</span>
        <div class="upload">
      ...
      <button class="fabu" @click="fabu">Release</button>
    </div>
    <div class="shuru" v-if="type==1">
      <div class="time">Release time</div>
      <input class="time1" type="date" v-model="message2.time1" />
      <div class="zhi">To</div>
      <input class="time2" type="date" v-model="message2.time2" />
      <button class="b1" @click="b1(1)">Search</button>
      <button class="b2" @click="b1(0)">All</button>
      <button class="b3" :class="xianzhi2==0 ? 'b6' : '' " @click="b3(0)">All</button>
      <button class="b4" :class="xianzhi2==1 ? 'b6' : '' " @click="b3(1)">Under sale</button>
      <button class="b5" :class="xianzhi2==2 ? 'b6' : '' " @click="b3(2)">Off the shelf</button>
      <table class="table">
        <tr>
          <th>Material name</th>
          <th>Category</th>
          <th>Price (Yuan)</th>
          <th>Original release time</th>
          <th>state</th>
          <th>Operation</th>
        </tr>
    ...
    <div class="shuru" v-if="type==2">
      ...
      <div>
        ...
        <label class="uploadfile" for="bjfile">Upload file</label>
      </div>
      <div>
        size
        <span style="position:relative;left: 120px;">Long</span>
    ...
    left: 600px;">
      ...
      <button class="fabu" @click="baocuntupian">Save changes</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    bianji(e, r) {
      ...
      if (this.message3.connect == 1) {
        this.message3.connect = "Single picture";
      } else if (this.message3.connect == 2) {
        this.message3.connect = "Two-sided continuity";
      } else {
        this.message3.connect = "Quadripartite continuity";
      }
      this.$forceUpdate();
    ...
    shangjia(e, r) {
      ...
      self.getData(obj).then(res => {
        if (res.data.status == 0) {
          ...
          this.message2.table.list[r].status = "Under sale";
    ...
    xiajia(e, r) {
      ...
      self.getData(obj).then(res => {
        if (res.data.status == 0) {
          console.log(res.data);
          this.message2.table.list[r].status = "Off the shelf";
        } else if (res.data.status == -95) {
          this.showJump = true;
          this.err = "Your account has expired ";
          setTimeout(function() {
    ...
    prev() {
      ...
      if (this.pageNumber <= 7) {
        ...
        if (this.cnt == 1) {
          this.showJump = true;
          this.err = "It's already the first page.";
          Scroll.stop();
          setTimeout(function() {
            self.showJump = false;
            self.err = "";
    ...
    next() {
      ...
      if (this.pageNumber <= 7) {
        ...
        if (this.cnt < this.pageNumber) {
          ...
          this.showJump = true;
          this.err = "This is the last page";
          Scroll.stop();
          setTimeout(function() {
            self.showJump = false;
            self.err = "";
        ...
        if (res.data.status == 0) {
          ...
          for (let i = 0; i < this.message2.table.list.length; i++) {
            ...
            self.message2.table.list[i].genre1 =
              self.hualist1[self.message2.table.list[i].genre1].english +
              "/" +
              self.hualist2[self.message2.table.list[i].styleType].english;
            ...
            if (self.message2.table.list[i].status == 0) {
              self.message2.table.list[i].status = "Under sale";
            } else {
              self.message2.table.list[i].status = "Off the shelf";
            }
          }
        }else if(res.data.status ==-95){
         this.showJump = true;
    ...
    RegisterStudio(e) {
      ...
      if (e == num) {
        ...
        if (self.message.lianjie == "Quadripartite continuity") {
          formData.append("connect", 2);
        } else if (self.message.lianjie == "Two-sided continuity") {
        ...
        self.getData(obj).then(res => {
          ...
          if (res.data.status == 0) {
            this.showJump = true;
            this.err = "Uploading pattern succeeded";
            setTimeout(function() {
              self.showJump = false;
              self.err = "";
            }, 1500);
    ...
    getFile1(e) {
      ...
      if (files.size > 5242880) {
        this.showJump = true;
        this.err = "The file size exceeds 5MB.";
        setTimeout(function() {
          self.showJump = false;
          self.err = "";
        }, 1500);
    ...
    getFile(e) {
      ...
      if (location > -1) {
        ...
        this.showJump = true;
        this.err = "Please select psd format file";
        setTimeout(function() {
          self.showJump = false;
          self.err = "";
        }, 1500);
    ...
    baocun(e) {
      ...
      if (num2 == e) {
        ...
        if (self.message3.connect == "Single picture") {
          formData.append("connect", 1);
        } else if (self.message3.connect == "Quadripartite continuity") {
          formData.append("connect", 2);
        } else {
          formData.append("connect", 3);
        }
        ...
        self.getData(obj).then(res => {
          if (res.data.status == 0) {
            console.log(res.data);
            this.showJump = true;
            this.err = "The modification is successful and will jump soon.";
            this.showLoading = false;
            setTimeout(function() {
              self.showJump = false;
              self.err = "";
  ...
  mounted() {
    ...
    this.getselect2();
    this.tablesucai();
    localStorage.setItem("path", "/EnFlowerManagement");
