<template>
  <div class="window">
    ...
    <div class="studio">
      <div class="click">Details of income and expenditure</div>
    </div>
    <div class="shuru">
      <div class="hello">
        <chart ref="chart1" id="echart1" :auto-resize="true"></chart>
      </div>
      <button class="b1" @click="tanchuang(1)">Binding bank card</button>
      <button class="b2" @click="tanchuang(2)">Withdrawals</button>
      <button class="b3" @click="xianzhi(0)" :class="xianzhi1==0?'xuanzhong':''">All</button>
      <button class="b4" @click="xianzhi(1)" :class="xianzhi1==1?'xuanzhong':''">Transaction income</button>
      <button class="b5" @click="xianzhi(-1)" :class="xianzhi1==-1?'xuanzhong':''">Cash withdrawal transaction</button>
      <div class="ul1">
        <div>Time</div>
    ...
    <div class="tanchuang" v-if="tan==1">
      ...
      <button @click="bdyh()">Binding bank card</button>
      <button @click="tanchuang(0)">Close</button>
    </div>
    <div class="tanchuang" v-if="tan==2">
      ...
      <div>
        <span>Cash withdrawal amount:</span>
        <input type="text" v-model="money" />
      </div>
      <button @click="tixian()">Withdrawals</button>
      <button @click="tanchuang(0)">Close</button>
    </div>
...
export default {
  ...
  methods: {
        ...
        if (res.data.status == 0) {
          ...
          for (let i = 0; i < self.message1c.list.length; i++) {
            ...
            if (self.message1c.list[i].type == 1) {
              self.message1c.list[i].type = "top-ups";
            }
            if (self.message1c.list[i].type == 2) {
              self.message1c.list[i].type = "Consumption";
            }
            if (self.message1c.list[i].type == 3) {
              self.message1c.list[i].type = "Direct consumption";
            }
            if (self.message1c.list[i].type == 4) {
              ...
              self.message1c.list[i].type =
                "Buy material NO" + self.message1c.list[i].indentId + "  " + a4;
            }
            if (self.message1c.list[i].type == 5) {
              ...
              self.message1c.list[i].type =
                "Sell material NO." + self.message1c.list[i].indentId + "  " + a5;
            }
            if (self.message1c.list[i].type == 6) {
              ...
              self.message1c.list[i].type =
                "Buy flower type NO." + self.message1c.list[i].indentId + "  " + a6;
            }
            if (self.message1c.list[i].type == 7) {
              ...
              self.message1c.list[i].type =
                "Sell pattern NO." + self.message1c.list[i].indentId + "  " + a7;
            }
            if (self.message1c.list[i].type == 8) {
              self.message1c.list[i].type = "Pay a personal studio deposit";
            }
            if (self.message1c.list[i].type == 9) {
              self.message1c.list[i].type = "Refund of personal studio deposit";
            }
            if (self.message1c.list[i].type == 10) {
              self.message1c.list[i].type = "Pay a deposit to the enterprise studio";
            }
            if (self.message1c.list[i].type == 11) {
              self.message1c.list[i].type = "Refund of deposit for enterprise studio";
            }
            if (self.message1c.list[i].type == 12) {
              self.message1c.list[i].type = "Withdrawals";
            }
          }
      ...
      self.getData(obj).then(res => {
        if (res.data.status == 0) {
          ...
          barChartBox.setOption({
            xAxis: {
              type: "category",
              data: ["Today's earnings", "Total revenue", "Withdrawal progress", "Current total assets"]
            },
            yAxis: {
              type: "value"
            },
    ...
    tixian() {
      ...
      if (self.message2.name == ""||self.message2.creditCardNumbers == ""||self.message2.bankOfDeposit == ""||self.message2.phone == "") {
        this.showJump = true;
        this.err = "Bank card information is blank.";
        setTimeout(function() {
          self.showJump = false;
          self.err = "";
        }, 1000);
      ...
      if (self.money < 100) {
        this.showJump = true;
        this.err = "The withdrawal amount shall not be less than ";
        setTimeout(function() {
          self.showJump = false;
          self.err = "";
        }, 1000);
        ...
        self.getData(obj).then(res => {
          ...
          if (res.data.status == 0) {            
            ...
            setTimeout(function() {
              self.showJump = false;
              self.err = "The withdrawal application was submitted successfully.";
              Scroll.move();
            }, 1000);}
          } else if (res.data.status == -95) {
            this.showJump = true;
  ...
  mounted() {
    ...
    this.getqianbao();
    this.getyh1();
    localStorage.setItem("path", "/EnMyLeader");
