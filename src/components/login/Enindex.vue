        ...
        <div class="content">
            ...
            <div class="formBox">
                <form action="" v-if="showArr">
                    ...
                    <div class="hBox">
                        <h2>Sign in</h2>
                        <p>No account? immediately<router-link to="/Enregister">Register</router-link></p>
                    </div>
                    <label for="">
                        <img src="../../assets/image/login/phone.png" alt="">
                        <input v-model="phoneNumber" placeholder="Phone" type="text">
                    </label>
                    <label for="">
                    ...
                    <!-- <div class="changeBox">
                        ...
                        <router-link :to='{path: "/Enforget",query: {number: phoneNumber}}'>Forget the password</router-link>
                    </div> -->
                    <button @click="toSubmit" class="btn">Sign in</button>
                </form>
                <form action="" v-if="!showArr">
                    <ul class="btnList">
                        <li @click="toOther(index)" v-for="(item,index) in newArr" :class="item.show?'btnLi':''" :key="index">{{item.name}}</li>
...
export default {
    ...
    data(){
        return {
            ...
            showArr: true,
            showLoading: false,
            newArr: [{name: 'Youchai Technology',show: true},{name: 'QFC Login',show: false}],
        }
    },
    methods: {
        toYouchai(){
        ...
        toOther(index){
            ...
            // this.generateUUID();
            // window.location.href = `https://my.tnc.com.cn/login.htm?clientCode=external_3dfitting&otherParam=3dfitting_${this.uuid}`
        },
        generateUUID(){
        ...
        changeCheck(){
            ...
            if(this.showCheck==true){
                localStorage.setItem('check',this.showCheck);
            }else{
        ...
        toSubmit(event){
            ...
            if(this.phoneNumber == '' || this.pass == ''){
                this.tig = "Necessary cann't be empty";
                this.tigShow = true;
                setTimeout(function(){
                    self.tig = '';
                    self.tigShow = false;
